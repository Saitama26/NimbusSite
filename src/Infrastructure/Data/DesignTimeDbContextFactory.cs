...
public sealed class DesignTimeDbContextFactory : IDesignTimeDbContextFactory<ApplicationDbContext>
{
    public ApplicationDbContext CreateDbContext(string[] args)
    {
        ...
        {
            // Пробуем найти .env файл в корне NimbusSite
            // Определяем путь к корню проекта (NimbusSite)
            var currentDir = Directory.GetCurrentDirectory();
            var infrastructureDir = Path.GetDirectoryName(typeof(DesignTimeDbContextFactory).Assembly.Location) 
                ?? currentDir;
            
            // Ищем корень NimbusSite (где находится .env файл)
        ...
        if (string.IsNullOrEmpty(connectionString))
        {
            throw new InvalidOperationException(
                ...
                "Please set it in your .env file (should be in NimbusSite/.env) or environment variables.");
        }
