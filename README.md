# NimbusClone - Multi-tenant Project Management System

NimbusClone - это система управления проектами с поддержкой мультитенантности, построенная на Clean Architecture и Domain-Driven Design (DDD) принципах.
...
2. Создайте файл `.env` в корне проекта (`tempRep/.env`) со следующим содержимым:

```env
# Database Configuration
DATABASE_CONNECTION_STRING=Server=localhost;Port=3306;Database=NimbusClone;User=root;Password=your_password;CharSet=utf8mb4;
MYSQL_SERVER_VERSION=8.0.21

# JWT Configuration
JWT_SECRET_KEY=your-super-secret-key-minimum-32-characters-long-for-security
JWT_ISSUER=NimbusClone
JWT_AUDIENCE=NimbusClone-Users
...
- Убедитесь, что файл `.env` находится в корне `tempRep/`
...
```sql
CREATE DATABASE NimbusClone CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
EXIT;
```

### Шаг 3: Применение миграций

Из корня проекта (`tempRep/`) выполните:
...
**Решение:** Убедитесь, что файл `.env` находится в корне `tempRep/` и содержит правильную строку подключения.
